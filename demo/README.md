...
its trajectory and switch between the two extracted surfaces — the
**conservative-iso** envelope (`iso = sqrt(3)/2 * h`, guaranteed to enclose every
stamped pose) and the **`iso = 0`** envelope (where sharp features can poke
through the marching-cubes reconstruction).
...
- **frame** slider (and **play** toggle) — scrub the arm through its 120-frame trajectory.
- **Envelope surface** radio buttons — `conservative iso` / `iso = 0` / `both`.
  Switch to `iso = 0` and scrub to a pose where the arm reaches a sharp extent to
  see the tool tip protrude past the surface; the conservative iso keeps it enclosed.
...
Reads the animated UR10 USD, stamps the swept-volume field (1800 poses, voxel
