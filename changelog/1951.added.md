...
`wp.mesh_query_*` builtins. `UniformSampler` also accepts an optional
`face_areas` argument so a caller can supply precomputed per-triangle areas
instead of having the sampler recompute them from the mesh.
